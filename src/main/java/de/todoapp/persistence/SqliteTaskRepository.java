...
public class SQLiteTaskRepository implements TaskWriter, TaskReader, TaskUpdater, TaskDeleter {
    ...
    public Task save(Task task) {
        ...
        String sql = """
            INSERT INTO tasks(id, title, description, due_date, status, category)
            VALUES (?, ?, ?, ?, ?, ?)
    ...
    public List<Task> findAll() {
        String sql = "SELECT id, title, description, due_date, status, category FROM tasks ORDER BY id";
    ...
    public Optional<Task> update(Task task) {
        ...
        try (Connection c = DriverManager.getConnection(url);
            ...
            if (task.getCategory() == null) ps.setNull(5, Types.BIGINT);
            else ps.setString(5, task.getCategory());
    ...
    private void bindTask(PreparedStatement ps, Task task) throws SQLException {
        ...
        if (task.getCategory() == null) ps.setNull(6, Types.BIGINT);
        else ps.setString(6, task.getCategory());
    }
    ...
    private Task mapTask(ResultSet rs) throws SQLException {
        ...
        String category = rs.wasNull() ? null : cat;
